...
function StudentDetail() {
  ...
  async function addLessons() {
    ...
    if (!Number.isFinite(n) || n <= 0) return toast.error("Enter a positive number of lessons");
    const purchased = (studentQ.data?.lessons_purchased ?? 0) + n;
    const { error } = await supabase
      .from("students")
      .update({ lessons_purchased: purchased })
      .eq("id", id);
    ...
    <div className="p-6 lg:p-10 max-w-5xl">
      ...
      <section className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <div className="text-2xl font-semibold text-slate-900">{completed}</div>
      ...
      <section className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${currentReadiness.cls}`}
            >
      ...
      <section className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {bookings.map((b: any) => (
            <div
              key={b.id}
              className="px-5 py-3 flex items-center justify-between text-sm flex-wrap gap-2"
            >
              <div>
