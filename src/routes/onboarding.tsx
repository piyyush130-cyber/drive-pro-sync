        ...
        <div className="glass-card w-full max-w-[560px] p-8 mt-8">
          {step === 1 && (
            <Step
              ...
              subtitle="This appears on your booking page and student communications."
            >
              <Input
                label="School name"
                required
                value={school.school_name}
                onChange={(v) => setSchool({ ...school, school_name: v })}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Input
                  ...
                  required
                  value={school.city}
                  onChange={(v) => setSchool({ ...school, city: v })}
                ...
                <div>
                  ...
                  >
                    {PROVINCES.map((p) => (
                      <option key={p} value={p} className="bg-[#0D1424]">
                        {p}
                      </option>
                    ))}
                  </select>
              ...
              <Input
                ...
                onChange={(v) => setSchool({ ...school, contact_phone: v })}
                placeholder="(204) 555-0100"
              />
              <Input
                label="Contact email"
                value={school.contact_email}
                onChange={(v) => setSchool({ ...school, contact_email: v })}
                placeholder="hello@yourschool.ca"
              />
              <Input
                label="Service area"
                value={school.service_area}
          ...
          {step === 2 && (
            <Step
              ...
              subtitle="These protect your time and set expectations with students."
            >
              <div>
                <Label>Cancellation notice required</Label>
                <select
                  className="glass-input"
                  value={rules.cancellation_notice_hours}
                  onChange={(e) =>
                    setRules({ ...rules, cancellation_notice_hours: Number(e.target.value) })
                  }
                >
                  <option value={12} className="bg-[#0D1424]">
                    12 hours
                  </option>
                  <option value={24} className="bg-[#0D1424]">
                    24 hours
                  </option>
                  <option value={48} className="bg-[#0D1424]">
                    48 hours
                  </option>
                </select>
              </div>
              ...
              <Toggle
                ...
                onChange={(c) => setRules({ ...rules, deposit_required: c })}
              />
              {rules.deposit_required && (
                <Input
                  ...
                  prefix="$"
                  type="number"
                  value={String(rules.deposit_cents / 100)}
                  onChange={(v) =>
                    setRules({ ...rules, deposit_cents: Math.round(Number(v) * 100) })
                  }
                />
              )}
              <div>
                ...
                <div className="space-y-2">
                  <RadioRow
                    ...
                    onChange={() => setRules({ ...rules, require_approval: false })}
                    title="Auto-confirm bookings"
                    desc="Booking confirmed instantly"
                  />
                  <RadioRow
                    checked={rules.require_approval}
                    onChange={() => setRules({ ...rules, require_approval: true })}
                    title="Manual review"
                    desc="Admin approves each booking"
                  />
                </div>
              </div>
            ...
            >
              <div className="space-y-2">
                {types.map((t, i) => (
                  <div key={i} className="grid grid-cols-12 gap-2 items-center">
                    ...
                    />
                    <select
                      className="glass-input col-span-3 text-sm"
                      value={t.duration_minutes}
                      onChange={(e) => {
                        const c = [...types];
                        c[i] = { ...t, duration_minutes: Number(e.target.value) };
                        setTypes(c);
                      }}
                    >
                      {[30, 45, 60, 90, 120].map((m) => (
                        <option key={m} value={m} className="bg-[#0D1424]">
                          {m} min
                        </option>
                      ))}
                    </select>
                    <input
                      ...
                      onChange={(e) => {
                        const c = [...types];
                        c[i] = { ...t, price_cents: Math.round(Number(e.target.value) * 100) };
                        setTypes(c);
                      }}
                    />
                    <button
                      onClick={() => setTypes(types.filter((_, x) => x !== i))}
              ...
              <button
                onClick={() =>
                  setTypes([
                    ...types,
                    { name: "", duration_minutes: 60, price_cents: 0, active: true },
                  ])
                }
          ...
          {step === 4 && (
            ...
            >
              ...
              <Input
                ...
                onChange={(v) => setInstructor({ ...instructor, email: v })}
              />
              <Input
                label="Phone"
                    ...
                    <div key={d.key} className="flex items-center gap-3">
                      <label className="flex items-center gap-2 w-32 text-sm">
                        <input
                          ...
                          checked={v.enabled}
                          onChange={(e) =>
                            setAvail({ ...avail, [d.key]: { ...v, enabled: e.target.checked } })
                          }
                      ...
                      {v.enabled ? (
                        <>
                          <select
                            ...
                            value={v.start}
                            onChange={(e) =>
                              setAvail({ ...avail, [d.key]: { ...v, start: e.target.value } })
                            }
                          >
                            {TIMES.map((t) => (
                          ...
                          <select
                            ...
                            value={v.end}
                            onChange={(e) =>
                              setAvail({ ...avail, [d.key]: { ...v, end: e.target.value } })
                            }
                          >
                            {TIMES.map((t) => (
                              <option key={t} value={t} className="bg-[#0D1424]">
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="glass-input"
          style={prefix ? { paddingLeft: 24 } : undefined}
        />
      </div>
    </div>
  );
  ...
  return (
    <label className="flex items-center justify-between cursor-pointer">
      ...
      <button
        ...
        className={`w-11 h-6 rounded-full transition ${checked ? "bg-[#3B82F6]" : "bg-slate-700"} relative`}
      >
        <span
...
function RadioRow({
  ...
  return (
    <button
      type="button"
      onClick={onChange}
      className={`w-full text-left p-3 rounded-lg border transition ${checked ? "border-[#3B82F6] bg-[#3B82F6]/10" : "border-slate-700 bg-transparent"}`}
    >
      <div className="flex items-start gap-3">
        <div
          className={`size-4 rounded-full border-2 mt-0.5 ${checked ? "border-[#3B82F6] bg-[#3B82F6]" : "border-slate-600"}`}
        />
        <div>
